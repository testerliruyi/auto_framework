...
class video_handle:
    ...
    def get_videos_format(path: str):
        ...
        if os.path.isdir(path):
            ...
            count = 0
            for file in files:
                ffprobe_cmd = 'ffprobe -i' + ' ' + file + ' ' + '-hide_banner'
            ...
            ffprobe_cmd = 'ffprobe -i' + ' ' + path + ' ' + '-hide_banner'
            result = subprocess.run(ffprobe_cmd, capture_output=True, text=True, encoding='utf-8')
            print("视频详细信息:", result)
            pattern = r"Video:[\s+]?\w+"
            res = re.findall(pattern, str(result))
    ...
    def convert_video_format(self, file_path, output_file_dir):
        ...
        if os.path.isdir(file_path):
            ...
            for file in files:
                ...
                ffmpeg_cmd = 'ffmpeg -i' + ' ' + file + ' ' + '-s 544x960 -b:v 512k' + ' ' + f'-vcodec {self.format}' + ' ' + str(
                    output_file_path)
            ...
            output_file_name = file_basename.split('.')[0] + f'_{self.format}' + '.' + file_basename.split('.')[1]
            output_file_path = os.path.join(output_file_dir, output_file_name)
            ffmpeg_cmd = 'ffmpeg -i' + ' ' + file_path + ' ' + '-s 544x960 -b:v 512k' + ' ' + f'-vcodec {self.format}' + ' ' + str(
                output_file_path)
            result = subprocess.run(ffmpeg_cmd, capture_output=True, text=True, encoding='utf-8')
            print(result)
...
if __name__ == "__main__":
    ...
    # video_handle().get_videos_format(r"E:\视频目录\output\video_ai_h264.mp4")
    video_handle().convert_video_format(r"E:\测试视频\video.mp4", r"E:\视频目录\output")
